###Importing Libraries
import cv2,sys,os,glob,errno,numpy as np,filetype,re,shutil
from os.path import isfile, join

##If command Line arguments are less, code will exit 
if(len(sys.argv)<3):
  print("Error less number of arguments ## EXITING")
  sys.exit()
scale_percent = int(sys.argv[2])
if(scale_percent<1 or scale_percent>99):
  print("Please Input Scaling Factor in 1 to 99")
  sys.exit()
##Reading file name
...
if(os.path.isfile(file_path)):
  print("File exists..Step 1 clear")
else:
...
if x:
  print("Your file is compatible Video File")
else:
  print("File Format is not video please try again")
  sys.exit()
...
if(os.path.exists(new_path)):
  inp = int(input("The raw images folder exists : \nPress 1 to start new,0 to end here: "))
  if(inp):
    os.chdir(new_path)
    files=glob.glob('*.jpg')
...
##Resized Images will be stored here in this path
...
for fil in glob.glob("*.jpg"):
  ...
  tempName = ("0"*(digits_files - len(str(i)))) + str(i)
  cv2.imwrite(os.path.join(out_image_dir,"out"+tempName+".jpg"),resized)
  i+=1
